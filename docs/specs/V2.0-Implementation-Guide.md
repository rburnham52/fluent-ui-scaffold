...
#### 2.2 Framework-Specific Options

```csharp
// src/FluentUIScaffold.Core/Configuration/FrameworkOptions.cs
namespace FluentUIScaffold.Core.Configuration
{
    /// <summary>
    ...
    public class SeleniumOptions : FrameworkOptions
    {
        ...
        /// <summary>
        /// Gets or sets the browser capabilities.
        /// </summary>
        public Dictionary<string, object> Capabilities { get; set; } = new();

        /// <summary>
        /// Gets or sets the remote web driver URL.
        /// </summary>
        public Uri? RemoteWebDriverUrl { get; set; }
    }
}
```
...
### Step 4: FluentUIScaffoldBuilder

```csharp
// src/FluentUIScaffold.Core/FluentUIScaffoldBuilder.cs
using FluentUIScaffold.Core.Configuration;
using FluentUIScaffold.Core.Interfaces;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Logging;
...
{
    /// <summary>
    /// Builder class for creating FluentUIScaffold instances.
    /// </summary>
    public static class FluentUIScaffoldBuilder
    {
        /// <summary>
        /// Creates a web application instance with the specified configuration.
        /// </summary>
        /// <typeparam name="TApp">The application type.</typeparam>
        /// <param name="configureOptions">The options configuration action.</param>
        /// <param name="configureFramework">The framework configuration action.</param>
        /// <returns>A FluentUIScaffoldApp instance.</returns>
        ...
        /// <summary>
        /// Creates a mobile application instance with the specified configuration.
        /// </summary>
        /// <typeparam name="TApp">The application type.</typeparam>
        /// <param name="configureOptions">The options configuration action.</param>
        /// <param name="configureFramework">The framework configuration action.</param>
        /// <returns>A FluentUIScaffoldApp instance.</returns>
        public static FluentUIScaffoldApp<TApp> Mobile<TApp>(
            Action<FluentUIScaffoldOptions> configureOptions,
            Action<FrameworkOptions> configureFramework = null) 
            where TApp : class
        {
        ...
        private static void ConfigureServices(
            ...
            Action<FrameworkOptions> configureFramework)
        {
            ...
            if (configureFramework != null)
            {
                ...
                configureFramework(frameworkOptions);
                services.AddSingleton(frameworkOptions);

                // Register the appropriate plugin based on framework options
                if (frameworkOptions is PlaywrightOptions)
                {
                    var plugin = new PlaywrightPlugin();
                    plugin.ConfigureServices(services, options);
                }
                else if (frameworkOptions is SeleniumOptions)
                {
                    var plugin = new SeleniumPlugin();
                    plugin.ConfigureServices(services, options);
                }
            }
            else
            {
                // Default to Playwright if no framework is specified
                var plugin = new PlaywrightPlugin();
                plugin.ConfigureServices(services, options);
            }

            // Register page components
            RegisterPageComponents(services);
        }

        private static void RegisterPageComponents(IServiceCollection services)
        {
            // This would typically scan assemblies for page components
            // For now, we'll register them manually
            var assemblies = AppDomain.CurrentDomain.GetAssemblies();
            foreach (var assembly in assemblies)
            {
                var pageTypes = assembly.GetTypes()
                    .Where(t => t.IsSubclassOf(typeof(BasePageComponent<>)) && !t.IsAbstract);
